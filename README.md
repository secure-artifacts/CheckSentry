...
同步只执行从 Google Sheets 到本地的单向下载，不会上传软件清单、浏览器插件、版本、安装路径、用户名或报告结果。云端存在的规则按对象身份更新本地同身份规则的所有字段（包括匹配字段、版本、发布者、所属工作表，以及备注/原因、添加人和添加时间），即云端无条件覆盖本地。云端没有的本地私有规则会保留在本地，因此本机发现的新软件仍需由用户手动复制到云端模板。网络不可用、链接无权限或表格结构不符合要求时，本次启动会跳过云端同步并继续使用本地清单。注意：云端表格中的超链接请使用纯文本形式填写，同步时为了兼容性会剥离 Google Sheets 原生 XML 超链接。
...
- 云端模板配置保存在本机被忽略的 `CheckSentry.cloud.json` 中；下载文件只存在临时目录并在导入后删除。
